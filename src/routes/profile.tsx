...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage your business profile, orders, and procurement preferences." },
    ],
  }),
          ...
          <div className="mt-4 flex flex-wrap items-center gap-5">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-3 text-sm opacity-80">
                <span className="flex items-center gap-1.5"><Building2 className="h-3.5 w-3.5" /> {user.company}</span>
        ...
        <div className="grid gap-8 lg:grid-cols-[240px_minmax(0,1fr)]">
          ...
          <aside className={`fixed left-0 top-0 z-50 h-full w-[min(86vw,320px)] bg-background p-5 premium-shadow transition-transform duration-300 lg:static lg:z-auto lg:h-auto lg:w-auto lg:translate-x-0 lg:bg-transparent lg:p-0 lg:shadow-none ${mobileNav ? "translate-x-0" : "-translate-x-full"}`}>
            ...
            <nav className="border border-border bg-card premium-shadow">
              ...
              <button
                onClick={() => { authStore.logout(); toast.success("Signed out"); navigate({ to: "/auth" }); }}
          ...
          <div className="animate-fade-in" key={tab}>
            {tab === "profile" && (
              <div className="border border-border bg-card">
                <header className="flex items-center justify-between border-b border-border px-6 py-4">
                  ...
                  <button onClick={() => setEdit((v) => !v)} className="inline-flex items-center gap-1.5 border border-border px-3 py-1.5 text-xs font-semibold uppercase tracking-wider hover:border-primary hover:text-primary">
                ...
                {edit ? (
                  <form onSubmit={onSave} className="grid gap-4 p-6 md:grid-cols-2">
                    ...
                    <EditField name="address" label="Shipping address" defaultValue={user.address} full />
                    <div className="md:col-span-2 flex gap-3">
                      <button type="submit" className="bg-primary px-5 py-2.5 text-xs font-semibold uppercase tracking-wider text-primary-foreground hover:bg-primary/90">Save changes</button>
                      <button type="button" onClick={() => setEdit(false)} className="border border-border px-5 py-2.5 text-xs font-semibold uppercase tracking-wider hover:border-primary">Cancel</button>
                    </div>
                  </form>
                ) : (
                        ...
                        <tr key={o.id} className="border-t border-border transition-colors duration-200 hover:bg-surface">
                          ...
                          <td className="px-5 py-3">
                            <span className={`px-2 py-0.5 text-[11px] font-semibold ${
                              ...
                              o.status === "Processing" ? "bg-amber-50 text-amber-700" : "bg-muted text-muted-foreground"
                            }`}>{o.status}</span>
                          </td>
                          <td className="px-5 py-3 text-right font-semibold">{formatINR(o.amount)}</td>
                          <td className="px-5 py-3 text-right">
                            <Link to="/order-tracking" className="text-xs font-semibold text-primary hover:underline">Track →</Link>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {tab === "wishlist" && (
              <div className="border border-dashed border-border bg-surface p-16 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">Save products to compare specifications and pricing for upcoming RFQs.</p>
                <Link to="/products" className="mt-5 inline-block bg-primary px-5 py-2.5 text-xs font-semibold uppercase tracking-wider text-primary-foreground hover:bg-primary/90">Browse catalog</Link>
              </div>
            )}
                ...
                <div className="mt-5 grid gap-4 md:grid-cols-2">
                  {[
                    ...
                    <div key={a.tag} className="border border-border bg-surface p-5 transition-all duration-300 hover:-translate-y-1 hover:border-primary hover:premium-shadow">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">{user.company} · GSTIN {user.gstin}</p>
                    </div>
                  ))}
                </div>
              </div>
...
function EditField({ name, label, defaultValue, full }: { name: string; label: string; defaultValue: string; full?: boolean }) {
  return (
    <label className={`block ${full ? "md:col-span-2" : ""}`}>
      <span className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{label}</span>
      <input name={name} defaultValue={defaultValue} className="mt-1.5 w-full border border-input bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none" />
    </label>
  );
}
